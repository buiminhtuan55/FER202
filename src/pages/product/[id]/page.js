...
export default function ProductDetail() {
  ...
  // Gọi fetchReviews khi load trang hoặc id thay đổi
  useEffect(() => {
    fetchReviews();
  }, [id]);
  ...
  const handleSubmitReview = async () => {
    ...
    const newReview = {
      ...
      userName: getDisplayName(currentUser), // <<< dùng hàm lấy tên mới
      rating,
      comment,
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Không thể gửi đánh giá. Server trả về lỗi.");
      }
    } catch (error) {
  ...
  const getDisplayName = (user) => {
    return user?.fullname || user?.username || "Ẩn danh";
  };
        ...
        <div className="bg-white p-4 mt-6 rounded shadow">
          ...
          {reviews.length > 0 ? (
            <div className="space-y-4">
              {reviews.map((review) => (
                <div key={review.id} className="border p-4 rounded relative bg-gray-50">
